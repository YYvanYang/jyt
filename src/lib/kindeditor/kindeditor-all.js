		...
		*/
		uploadbutton.init({
	        ...
	        'onUploadSuccess': function(file, data, response){
				dialog.hideLoading();
				var data=eval('(' + data + ')');
				if (data && data.resultCode == 4){
					var url = data.filePaths.pop()||"";
					...
					if (!fillDescAfterUploadImage) {
						clickFn.call(self, url, data.originalFileNames.pop()||"", data.width, data.height, data.border, data.align);
					} else {
						K(".ke-dialog-row #remoteUrl", div).val(url);
						K(".ke-tabs-li", div)[0].click();
						K(".ke-refresh-btn", div).click();
		...
		var settings = {
			...
			upload_success_handler : function(file, serverData) {
				...
				try {
					...
					//added by francklin
					data = {
						error: data1.resultCode==4 ? 0: 1,
						message: data1.resultMsg,
						url: data1.filePaths.pop()||""
					}
				} catch (e) {
